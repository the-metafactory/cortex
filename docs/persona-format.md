...
| `allowedTools` | string[] | Tool allowlist for the agent. **Free-form string list in v1** — cortex does not validate against a closed enum. The canonical v1.0 tool names cortex knows about are: `Read`, `Edit`, `Write`, `Grep`, `Bash`, `Glob`, `Agent`, `Skill`, `WebFetch`, `WebSearch`, `NotebookEdit`, `TodoWrite`, `BashOutput`, `KillShell`. Future tool additions land via additive enum extension (minor bump). **Enforced since cortex#2386 (EBH-7a) for `DispatchHandler`-routed dispatch:** every canonical tool NOT in the declared list is merged into the session's `effectiveDisallowed` set — the same seam `agentDisallowedTools` (WEB-2/B1) uses, not a separate mechanism — and declaring the field at all also adds `--strict-mcp-config` (the canonical set never names an `mcp__*` tool, so a declared allowlist implies "no MCP" too). An explicit `allowedTools: []` is a real declaration — it denies every canonical tool, it is NOT "no restriction". Omitting the field entirely leaves the agent unaffected by this mechanism — absence of a declaration is not a declaration of nothing. Unknown tool names (not in the canonical list, including typos): grant nothing and deny nothing; a warning is logged with the name so a persona author can catch a typo that silently failed to grant the tool they meant. **Known gap:** this enforcement lives in `dispatch-handler.ts` and does NOT cover agents dispatched through the web-gateway path (`src/gateway/bus-inbound-sink.ts`) — that path bypasses `DispatchHandler` entirely (cortex#1758) and remains a separately-tracked hole; see `docs/security/hardening-plan.md`. |
...
| `allowedTools` references a tool cortex doesn't know about | Warning logged; unknown tool name is preserved on the agent; other tool names register normally. Since cortex#2386, the same unknown name is ALSO logged (to stderr) at dispatch time by `dispatch-handler.ts`, because it can't be projected onto a real deny/allow decision — the name grants nothing and denies nothing. |
