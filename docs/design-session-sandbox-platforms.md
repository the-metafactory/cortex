...
| ~~**OQ-5**~~ | ~~Add systemd hardening (`ProtectHome=`, `ReadOnlyPaths=`) to `cortex@.service` as a cheap complementary layer?~~ | ✅ **RESOLVED — shipped, and the real Linux boot test (`scripts/__tests__/systemd-render-e2e.test.ts`, the "systemd e2e (systemd-user session)" CI job) caught a real regression before it reached anyone.** `src/services/cortex@.service` carries `NoNewPrivileges=yes`, `PrivateTmp=yes`, `ProtectSystem=strict` + `ProtectHome=read-only` (chosen over `true`/`tmpfs` — the daemon reads broadly under `$HOME` and a full hide/replace risked breaking an unenumerated legitimate read) with `ReadWritePaths=` punched for the three trees it actually writes (`~/.config/metafactory/cortex`, `~/.local/share/metafactory/cortex`, `~/.local/state/metafactory/cortex`) plus `~/.claude/events/raw` (the hook-substrate write target), and `RestrictSUIDSGID=yes`. **Genuinely useful constraint the e2e surfaced (not obvious in advance): `cortex@.service` is a `systemctl --user` unit, and a per-user systemd manager has no host `CAP_SYS_ADMIN`/`CAP_SYS_MODULE` — so `ProtectKernelTunables=`, `ProtectKernelModules=`, and `ProtectControlGroups=` are SYSTEM-UNIT-ONLY.** The first attempt shipped all three; the real e2e job caught `cortex@e2e2071.service: Main process exited, code=exited, status=218/CAPABILITIES` with `journalctl` showing `Failed to drop capabilities: Operation not permitted` — the daemon never started, on every boot, not a degraded-but-working state. All three were removed (comments in the unit explain what each would have bought and that they need a system unit); `RestrictSUIDSGID=yes` was kept (pure seccomp — installable by a process on itself without host capabilities, mechanically unlike the removed trio) and the e2e confirmed it's fine in a `--user` unit. **This is a real constraint for the future L2/system-unit conversation too**: if cortex (or a sibling app) ever ships a privileged system-unit variant, the kernel/cgroup trio becomes available again and should be revisited then — but a `--user` unit structurally cannot have them, no matter how they're tuned. Two directives were deliberately **not** set: `MemoryDenyWriteExecute` stays `no` (explicit, not omitted) — bun/node's JIT needs W+X pages, `yes` would SIGSYS the runtime; `RestrictNamespaces=` is **omitted entirely** — the not-yet-built `linux-bwrap` backend (E5/E6) will need to create its own namespaces as a child of this same daemon, and its exact minimal set isn't pinned yet, so guessing a value now risks silently blocking L2 the day it ships. Revisit `RestrictNamespaces=` alongside EBH-3b once that set is known. A second, independent regression the e2e/Test run also caught: an `ExecStartPre` `+`-prefix experiment (meant to pre-empt a hypothesized "`ReadWritePaths=` requires the target to pre-exist" first-boot failure) broke a pinned literal-string assertion in `scripts/__tests__/systemd-render.sh` — reverted to the plain (sandboxed) `ExecStartPre=` form, which the e2e confirms works: `ReadWritePaths=` does not require the mountpoint to pre-exist. |
