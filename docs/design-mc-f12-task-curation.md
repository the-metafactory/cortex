...
**No side panel, no separate `/curate` route, no separate task-detail page.** The dashboard has a single primary surface (the focus area + working grid + task table) and a single overlay (the drill-down). Adding a third surface to host curation buttons trades one click for two screens of operator cognitive load — the wrong trade. The drill-down is already where the operator goes when they think *"this task is stuck / wrong / done with the wrong agent"*; the verbs they need at that moment belong on that screen.
...
| `completed` | ❌ terminal | ❌ rerun creates new assignment via Dispatch | ❌ task already done | ✅ (operator may close out the task even from completed) |
...
**`Dispatch` enablement.** Enabled in two cases: (a) the task has zero assignments (empty-assignment), or (b) the task's primary active assignment is in a terminal state (`completed`/`failed`/`cancelled`) and the operator wants a fresh re-run on the same task. For terminal-state Dispatch, the new row is a fresh `agent_task_assignment` — the existing terminal row is preserved as historical record (matches F-8 Decision 4's handling of terminal-state assignments rendering with strike-through-on-hover).
...
**`Hand off` enablement.** Legal from any non-terminal state on the assignment row, including `failed` (the operator says *"that agent gave up; let a different one try the same task"*). Disabled on `completed` and `cancelled` because there's no in-flight work to hand off. From terminal-but-rerunnable states (`failed`), Hand off is semantically equivalent to "Dispatch with a new agent" — Decision 6 pins the implementation: hand-off from a terminal state creates a new assignment row to the new agent (same as Dispatch), hand-off from a non-terminal state cancels the current assignment and creates a new one (true mid-flight reassignment).

**`Abandon` enablement.** Legal from any non-terminal state (`queued`/`dispatched`/`running`/`blocked`) on the assignment AND legal from `completed` on the task (operator closing out). Disabled on `cancelled` (task already cancelled). Decision 5 pins what "abandon" actually does at the row level.

**Disabled buttons render with tooltips.** Each disabled button gets a one-line tooltip from the matrix above ("Already running — cannot dispatch", "Task already cancelled", etc.) so operators learn the model from the UI without reading docs.
...
**Agent list source.** The full `agents` table — every row, regardless of `type` (`head`/`hands`) or `persistent` flag. Rationale: F-12 is operator-facing, not policy-driven, and the operator may legitimately dispatch to either type. Filtering would hide the available choice without making it safer (any agent the operator can name in the dropdown was already a valid dispatch target via the existing `POST /api/sessions` flow).

**Default.** `mc-default-agent` (the well-known row created lazily by `ensureDefaultAgent` in `src/mission-control/api/handlers.ts:257`). The operator can override before clicking. If multiple agents exist, the dropdown sorts by `agents.name ASC` with the default pinned to the top.
...
**Why not a tighter "head-agents-only" filter.** Concern 5 in the parent spec explicitly retired the `pets/cattle` taxonomy and replaced it with `persistent: bool` + `head/hands`. F-12 does not re-introduce a policy on top — the `type` column is metadata for the operator's mental model, not an access-control gate.
...
**Confirmation dialog (Decision 8 covers the full posture).** Yes, abandon prompts for confirmation. Free-text reason field is optional. The reason becomes the payload of the `operator.curation` audit event (Decision 9).

**What about cascading?** Abandon-the-task does **not** cascade-cancel any other assignments on that task. Tasks can have multiple assignments (Concern 2 Q2.4 / §3.3); the operator may have a `running` assignment on the same task and Abandon should not blindly take it down. F-12's button operates on the **drill-down's primary active assignment**, full stop. If the operator wants to nuke all assignments on a task, they Abandon each one in turn. This is intentional restraint — bulk operations are explicitly out of scope (Decision 10).
...
§3.3 describes two `operator_requeue` transitions: `blocked → queued` and `failed → queued`. The state machine implements both (`src/mission-control/state-machine.ts:52,59`). F-12's Requeue button is the operator's UI handle on these.
...
**Wait, doesn't that strand the assignment in `queued`?** Yes, until Dispatch fires. F-12's UX surfaces this clearly — after Requeue, the toolbar refreshes (Decision 3 matrix) and the operator sees `[Dispatch]` enabled while the other buttons return to their `queued`-state enabled state. An operator who Requeues a `failed` assignment and walks away gets a `queued` assignment that sits forever until either the F-13 dispatcher picks it up or another operator hits Dispatch. This is desirable: Requeue is *"this assignment was wrongly terminated; put it back on the runway"* — it's not *"and start it again right now"*. The latter is exactly what Dispatch on a terminal-state assignment does (Decision 4); operators who want both effects in one click should be told to use Dispatch directly.

**What about the `events` row?** The existing `applyTransition` already inserts a `state.transition` event with `payload.action = 'operator_requeue'`. F-12 adds a sibling `operator.curation` event (Decision 9) for the audit-trail-with-rationale shape. Two events for one operator action — the `state.transition` is the state-machine truth, the `operator.curation` is the operator-rationale truth. Both flow through the same `broadcastEvent` plumbing (`notifications.ts:43`).
...
| Requeue | ❌ — reversible (operator can Abandon the requeued assignment if they change their mind) | ❌ |
| Hand off | ✅ — semi-destructive (cancels in-flight assignment) | ✅ optional |
| Abandon | ✅ — destructive (cancels assignment or task) | ✅ optional |

**Reason field.** Optional, single-line text input under the confirm prompt. When non-empty, the string lands in the `operator.curation` event's payload (Decision 9). When empty, the event still fires — just without rationale text. Operators who skip the field are not penalised; the audit row exists either way.
...
**Keyboard.** `Esc` closes the confirm panel without acting; `Enter` confirms (matches F-10 input submit semantics). The reason field captures `Tab` correctly so the operator can flow Abandon → Tab → reason → Enter without a mouse.
...
## Decision 9 — Audit trail: a single `operator.curation` event family with a `kind` discriminator
...
**Why not extend `operator.input` with a `payload.action` discriminator.** The user's review bar called out `operator.input` (`src/mission-control/db/events.ts:175,205`) as the established home for operator-authored events and asked why the curation verbs aren't members of that family. Three reasons F-12 keeps `operator.curation` separate:

- **Semantic conflation.** `operator.input` is an operator **utterance** — text the operator typed that the agent will read. Decision 3 of `events.ts` calls this out explicitly: *"operator.input is the authoritative H-source"*, meaning it's the human-channel input the agent acts on. Curation verbs are operator **actions on state** — they don't feed the agent, they reshape the funnel around it. Folding them into one type forces every `operator.input` consumer (dashboard renderer, future replay tooling, audit export) to branch on `payload.action` to decide *"is this text-the-agent-saw or a button-the-operator-clicked"*. That branch is wider than the curation-only `kind` branch because it also forks rendering (utterances render as chat bubbles; verbs render as state-change rows), attribution (utterances belong in the input-affordance lineage; verbs belong in the curation-toolbar lineage), and replay semantics (utterances would re-trigger agent behaviour on replay; verbs would not).
- **F-7 renderer branching cost.** F-7's drill-down renders `operator.input` as a chat bubble in the event log timeline. Reusing the type for curation verbs would either render four new payload variants as chat bubbles (semantically wrong — the operator did not "say" anything) or fork the renderer on `payload.action` to choose bubble vs state-change. That fork is the same one Decision 9's `operator.curation` block does, except the chat-bubble code path remains pristine and the curation block is its own self-contained renderer.
- **Multi-operator attribution at Tier 2.** §10's Tier 2 work introduces per-event `operator_id` columns. When that lands, `operator.input` and `operator.curation` will get the column wired in independently: utterances are attributed to whoever typed them, verbs are attributed to whoever clicked them. A unified type would have to negotiate the same column twice (once for each semantic), and any future per-family policy (e.g. *"only senior operators can fire `kind: handoff`"*) would have to gate inside `operator.input`'s authorization path — coupling utterance permissions to verb permissions.

The deviation from the bar is therefore deliberate: `operator.curation` is a sibling family, not a subset, and the `kind` discriminator scopes to the curation surface only. If a fifth verb lands (e.g. F-12b's `kind: "import"`), it joins the curation family; if a fifth utterance lands (e.g. an annotation surface), it joins `operator.input`. Two clean families beat one overloaded one.

**Where it's inserted.** Each new endpoint (`POST /api/sessions` already exists for Dispatch; `POST /api/assignments/:id/requeue`, `POST /api/assignments/:id/handoff`, `POST /api/assignments/:id/abandon` for the other three) inserts the `operator.curation` event after the state-machine action commits, via a new helper `createOperatorCurationEvent(db, sessionId, payload)` in `src/mission-control/db/events.ts` — sibling of the existing `createOperatorInputEvent` (`events.ts:194`) and `createPermissionRequestEvent` (`events.ts:213`).
...
**WS broadcast.** Each `operator.curation` event flows through `broadcastEvent` (`notifications.ts:43`) to connected dashboard clients. The drill-down's event log adds a render block keyed on `payload.kind` rendering one of four one-line summaries:
...
**Operator id on the event.** Schema-wise `events` doesn't carry `operator_id` — it inherits the operator via the assignment → task chain. F-12 doesn't introduce per-event operator id (deferred to Tier 2 multi-operator runtime per §10). The `operator.curation` event is implicitly attributable to the single operator on Tier 1.
...
- **Task comments / discussion thread.** Operators may want a free-form comment per task ("paused this until upstream lib fixes the race"). The existing F-7 drill-down's input affordance (F-10) already lands operator-authored text in `operator.input` events — operators can use it as a comment surface today by typing into a paused/queued assignment's input. A dedicated comment column on `tasks` is out of scope.
- **Task tagging / labels.** GitHub-style labels on tasks would be a useful filter axis. Out of scope. v1 priority + state filters cover the active operator's working set; tagging is a post-Tier-2 expansion alongside multi-operator.
...
- **Per-operator curation history view.** "Show me everything I curated today" — useful for retro / audit but additive over the F-7 drill-down per-assignment view. Out of scope. The `operator.curation` events are queryable from the DB if anyone needs them.
...
- **Confirmation dialog for Dispatch.** Decision 8 explicitly skips it (additive verb, low blast radius). If operators report pebcak Dispatches in practice, this is a one-line UI change post-merge — but no speculative gating in v1.
...
- Three new endpoints in `src/mission-control/api/handlers.ts`:
  - `POST /api/assignments/:id/requeue` — calls `applyTransition(... operator_requeue)` + emits `operator.curation` event with `kind: "requeue"`.
  - `POST /api/assignments/:id/abandon` — body `{ scope?: "assignment" | "task"; reason?: string }`. When `scope === "assignment"` (default if assignment is non-terminal), calls `applyTransition(... cancel)`. When `scope === "task"` (default if assignment is terminal or absent), updates `tasks.status = 'cancelled'`. Emits `operator.curation` event with `kind: "abandon"`.
  - `POST /api/assignments/:id/handoff` — body `{ newAgentId: string; reason?: string }`. Cancels the current assignment (if non-terminal) via `applyTransition`, creates a new `agent_task_assignment` row, spawns a controlled session via existing `spawnControlledSession`. Emits `operator.curation` event with `kind: "handoff"`.
- Existing endpoint `POST /api/sessions` is the Dispatch backing — no changes to its surface; the dashboard wires the existing endpoint into the new `[Dispatch]` button. One new `operator.curation` event with `kind: "dispatch"` is inserted alongside the existing `state.transition` events the spawn already produces.
...
- Dashboard changes in `src/mission-control/dashboard/index.html`:
  ...
  - New render branch for `operator.curation` events in the F-7 event log (Decision 9's four one-line summaries).
...
- Per-operator history view → post-Tier-2 (Decision 10).
- Confirmation dialog on Dispatch → revisit if operators report mistakes (Decision 10).
...
- [ ] Each curation endpoint inserts an `operator.curation` event with the Decision 9 payload shape and broadcasts it via `broadcastEvent`.
...
- [ ] The F-7 drill-down event log renders one of four one-line summaries for `operator.curation` events per Decision 9.
...
- Dashboard edits in `src/mission-control/dashboard/index.html`:
  ...
  - New render block for `operator.curation` events in the event log.
- Tests:
