...
- **F4 — Principal-DM integrity.** The guard-off DM context is only as safe as (a) no relay path forwarding untrusted content into it and (b) the principal↔platform-ID mapping being immutable config. Treat the mapping as security-critical; land G-301 as a real, filed issue (the `#42` citation here and in the review is STALE — `#42` is an unrelated, merged MIG-7 migration PR; G-301 does not currently exist as trackable work, per the EBH-6 investigation below).
