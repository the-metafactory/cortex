...
**3. The standard member bind is operator-mode account-bound; creds-only on `$G` is the documented fallback.** `resolveLeafBindMode` (`leaf-remote-renderer.ts`) renders the leaf from the same sealed `.creds` two ways, chosen by the member's **local** bus type:
