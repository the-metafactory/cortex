...
**2. The hub mints your *transport* credential, never your *identity*.** This is the precise reconciliation with ADR-0013. ADR-0013 rejected hub-minted **identity** (authenticating *as* an account inside someone else's operator for who-you-are). The payload swap hub-mints only a **transport user** (a narrowly-scoped user in the hub's FED account, for pipe auth) — the member's signing key and their own local account stay theirs. **Transport-credential issuance ≠ identity issuance.** ADR-0013's "no hub-minted credentials" is hereby narrowed to "no hub-minted **identity**"; hub-minted transport creds are the delivered, accepted mechanism.

**3. Operator-mode account-bound is the standard member bind; creds-only on `$G` is the documented fallback.** `resolveLeafBindMode` (`leaf-remote-renderer.ts`) renders the leaf from the same sealed `.creds` two ways, chosen by the member's **local** bus type:

- **operator-mode local bus (STANDARD):** the leaf remote carries an `account:` line binding imported subjects to the member's **own** local FED account (in the member's own operator). This is what `jc/default` and `jc/clawbox` (on AC25) run; `cortex network provision` + `make-live` stand the bus up.
- **`$G`/default local bus (FALLBACK):** no `account:` line — subjects land in `$G`, the binding rides in the `.creds` JWT. Mechanically valid and non-destructive, but the standard is operator-mode; the SOPs guide a member onto operator-mode with their own FED account.

The scoped `.creds` (hub-account transport user) authenticates the pipe in **both** modes; the bind mode only decides the local `account:` line.
