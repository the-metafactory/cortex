...
The confusion was structural: the credential *form* changed, the *minting authority* changed, and a `$G`/creds-only bind became mechanically possible — but the docs still described the ADR-0018 PSK world, and `sop-network-join.md:30` even mislabels the newly-possible creds-only path as rejected (conflating a fallback bind with hub-minted identity). This ADR records the delivered model as the single anchor the docs align to.
...
- **CONTEXT.md and both SOPs are corrected** to the scoped-`.creds` transport, the hub-mints-transport-not-identity reconciliation, and operator-mode-standard / creds-only-fallback. `sop-network-join.md:30`'s "creds-only = rejected" mislabel is corrected: creds-only is a *fallback bind mode*, not hub-minted identity (which is the thing that is actually rejected).
...
- **Hub-minted identity account — REJECTED (ADR-0013, unchanged).** The hub minting *who you are* collapses sovereignty. The delivered model deliberately mints only a scoped *transport* user; identity stays the member's own `SU` key.

## Note on retired language

This ADR **retires the "Model A / Model B" framing** (ADR-0013) from the living docs. That design-time dichotomy stopped mapping cleanly once the delivered model added hub-minted *transport* creds — "Model B = nothing hub-minted" became false, and "Model B, narrowed" was the source of recurring "are we reviving Model A?" confusion. Describe the layers **directly** instead: federation is **sovereign** (your identity/`SU` key is your own, never hub-minted); the leaf's **transport credential** is hub-issued. The rejected pattern is **hub-minted identity**, named as such. ADR-0013 keeps its original wording as the historical record; new/living docs (CONTEXT.md, SOPs, this ADR) do not use "Model A/B" as live vocabulary.
