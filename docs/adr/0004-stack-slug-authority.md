...
2. The **`cortex network join` write path** — where join writes resolved policy. **(Amended by DA-5, #813:** this targets the file the daemon composes policy from, resolved from its `--config` locator — convergent with `stack.id` under the alignment invariant, see DA-5.**)**
...
- **DA-5 — The `cortex network join` write path targets the file the daemon composes policy from (resolved from its `--config`), NOT a `stack.id`-derived path.** Under the alignment invariant (DA-1: locator == `stack.id`) these are the same file; on a drifted stack the join follows the locator so the write stays effective (no split-brain), while the drift is surfaced by `warn_stack_identity_drift` (DA-2) for reconciliation (DA-4). Locator-resolved, `stack.id`-convergent. This refines item (2) of §Context, which originally listed the join write path among the things derived directly from `stack.id`: that held only under alignment, and on JC's drifted single-file stack it produced the #805 split-brain (join wrote a `stack.id`-slug path the daemon never read). The join now resolves its target from the daemon's real `--config` (the file `composeRawConfig` reads `policy:` from), with a fail-closed guard that refuses to write when no running daemon loads that `--config` (no orphan policy block). Refs #805/#807/#813.
...
- **The join write path no longer split-brains on a drifted stack (DA-5, #813).** Because the join now resolves its policy target from the daemon's `--config` rather than a `stack.id`-derived path, a drifted stack's policy lands in the file the daemon actually composes — the leaf links instead of silently no-op'ing. The drift itself is still surfaced by `warn_stack_identity_drift` (DA-2) and reconciled by a principal rename (DA-4); the join is correct in the interim rather than waiting on the rename. A fail-closed guard turns a mis-passed `--config` into an actionable abort (no orphan), rather than a second silent split.
