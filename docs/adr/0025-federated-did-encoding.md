...
## Compatibility — the hard part, and one OPEN question for the co-signers
...
**The OPEN question — hard cut vs dual-accept — is NOT resolved here, by design.** There is a direct tension between two ratified-at-different-times artifacts, and reconciling it is a co-signer decision:

- **cortex#1880's own acceptance criteria** (authored 2026-07-11) require, *if the encoding changed*, a **dual-accept window**: "old-form stamps still verify for one release, and old-form usage is logged." Compass `sops/federation-wire-protocol.md` sets the same dual-accept default for any wire-contract change.
- **RFC-0001 §9** (decision Andreas 2026-07-12; Ratified single-principal 2026-07-13) mandates a **HARD CUT**: "NO dual-registration, NO staged emitter window, NO ongoing legacy verifier tolerance," and states it "supersedes any dual-accept-window language" and that "the dual-accept default of compass `sops/federation-wire-protocol.md` was considered and deliberately overridden (§8.9)." The runbook §3 (atomic cut) and §7/§9.3 (scoped `[principal-hands]` purge) are written to the hard cut.

These cannot both hold. **This ADR does not pick.** It records the tension for the co-signers because (a) the myelin ratification that mandated the hard cut was **single-principal** (Andreas, 2026-07-13), whereas the *federated* wire is cross-principal, so JC has not yet co-signed the hard-cut posture for the shared wire; and (b) a hard cut discards cross-principal signed history, a consequence JC's stack bears directly.

> **OQ-1 (blocking the WP-4 build, for Andreas + JC).** Does cortex honour RFC-0001 §9's **hard cut** (no dual-accept; pre-cut signed history discarded; single coordinated release R per the runbook), or does it hold to **#1880's dual-accept acceptance criterion** (old-form stamps verify for one release)? The two are mutually exclusive on the federated wire.
> - If the co-signers affirm the **hard cut**: cortex#1880's dual-accept acceptance criterion and verification bullet are **superseded** and must be struck/amended on the issue (they predate the 2026-07-12 §9 decision), and the WP-4 build carries **no** dual-accept window — the cut is the runbook's atomic flip (§3) + scoped purge (§7).
> - If the co-signers require a **cortex-local dual-accept window** despite §9: that is a deliberate cross-principal divergence from the ratified RFC and must be recorded back into RFC-0001 §9 / the runbook so emitter and verifier do not flip atomically — cortex cannot unilaterally run a dual-accept window against a hard-cut peer without the peer's verifier tolerating old-form stamps too.

**Reconciliation with the runbook.** Whichever disposition the co-signers choose, this ADR is the record that runbook §1.6 gates on. On co-sign, the runbook §1.6 "DECISION PENDING" marker and the §Sign-off "Blocking decision still open" note are cleared to cite ADR-0025, and §3/§7 proceed under the chosen posture.
...
This ADR is a **cross-principal wire contract** and lands only when **both** principals co-sign. Until then it is `proposed`; do not merge, do not self-sign.

| Role | Principal | Signed (name + date) |
|---|---|---|
| Decision owner | Andreas | `<REPLACE_ME>` |
| Co-signer (hub custodian) | JC | `<REPLACE_ME>` |

**Blocking open question at authoring time:** OQ-1 (hard cut vs dual-accept, §Compatibility) — the co-signers must resolve it as part of signing; a signature affirms the disposition recorded above it.
