...
| **PR-4** | Schema update: `agents[].capabilities` + `agents[].sovereignty` + `agents[].maxConcurrent` on `CortexConfigSchema`. Schema validation tests. Backwards-compat (existing cortex.yaml without these fields stays valid). | None | ~50 + 100 tests | Config-loader tests; round-trip parse | tsc, bun test |
...
| The `capabilities` field on cortex.yaml is misconfigured (e.g. typo `code-revew.typescript`) and silently no one claims the typo capability | High (config typos are common) | Low — pilot times out, operator notices | v1 ships a startup log line listing all registered capabilities. v2 adds a JSON Schema validator on the capabilities field (regex `code-review\.[a-z]+`). Tracked in §13. |
...
The cortex#249 `DispatchTaskFailedReason` taxonomy has four nak kinds (`cant_do`, `wont_do`, `not_now`, `compliance_block`) — none of which cleanly represents "the substrate (CC session) crashed mid-review through no fault of the agent's intent." We use `cant_do` for this case in §7.6 as the closest fit, but the architectural ideal is to extend `DispatchTaskFailedReason` with a `substrate_failed` discriminator.

Open: should we extend the taxonomy in cortex#237's PR cluster, or file a separate cortex issue (call it `cortex#251`) for the extension and have cortex#237 use `cant_do` in the interim?

Recommendation: separate issue. Cortex#249 just merged; reopening the taxonomy in this consumer's PR cluster expands scope. Use `cant_do` in v1, file a follow-up issue, and update the consumer to use `substrate_failed` once the extension ships. Track in §13.
