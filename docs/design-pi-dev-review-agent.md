...
**Related docs:** `docs/architecture.md` (§3 event architecture + bus contracts; §4 cortex's contract with each layer; §9 agent + presence/renderer model), `docs/design-arc-agent-bots.md` (substrate-pluggable bot packaging via arc — the natural follow-on to this doc). Earlier drafts referenced `docs/design-event-taxonomy.md` and `myelin/docs/design-agent-task-routing.md`. Neither is a checked-in cortex document: the event-taxonomy draft was never ported from grove-v2 (see `docs/plan-cortex-migration.md` §Reading), and the task-routing model is tracked in myelin issue #36. The event vocabulary lives in `docs/architecture.md` **§3** (event architecture) and `src/bus/{system,dispatch}-events.ts`; the dispatch lifecycle contract is **§7.3**. *(Corrected 2026-08-11 — both the earlier "lives inline in §6" pointer and this line's own opening "§6 bus contracts" were wrong: §6 is M7 application architecture.)*
