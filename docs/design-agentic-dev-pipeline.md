...
- **FAIL CLOSED when no forge identity is configured (cortex#2436 + #2438, 2026-07-26).** `wireDevConsumers` resolves an identity in precedence order and returns **no consumers** when it finds none — nothing binds, nothing pushes, and the dev loop is inert on that stack until one is configured (queued dispatches stay on the stream and run once it is).

  1. **`agents[].github.identityName`** — the durable model. A short-lived credential is minted **per forge operation** (`dev-consumer-boot.ts` `resolveToken`), so expiry is a non-issue.
  2. **`CORTEX_DEV_GH_TOKEN`** — a static override, retained for stacks on a non-App credential and for tests. Long-lived by nature, so supported rather than recommended.

  The declared identity **wins** when both are present: silently preferring the static token would mean config claims a bot identity while pushes carry a long-lived credential.

  This **supersedes the original ambient-fallback behaviour**, in which the consumer wired anyway and pushed under the daemon's ambient `gh` credential — the principal's own account — behind a boot warning. That was accepted as the F-2 residual below, but a warning is not a control, and the chat dispatch path already refuses in the same situation ([#2408](https://github.com/the-metafactory/cortex/pull/2408)). Two paths answering *"which identity does this agent act as?"* with **opposite failure modes**, selected by how the work arrived and invisible from `agents[]`, is worse than either alone.

  Note the bullet above recommended "machine-user PAT" as the intended resolution: [vision#11](https://github.com/the-metafactory/vision/issues/11) superseded that with per-assistant GitHub App identities, and the durable forge-neutral credential model is `docs/design-forge-neutral-agent-identity.md` (cortex#2423). Its **M8** removes the `CORTEX_DEV_GH_TOKEN` boot read entirely once the credential helper (M3/M4) lands; until then the static override above remains as the lower-precedence fallback.

- **The honest F-2 caveat (narrowed, not retired):** until F-5b sandboxing lands, `dev.implement` executes on a local machine, so OS-level ambient authority is bounded only by the guardrails above plus the caps in §3.5. That residual is accepted for v1 on the principal's own stacks and is precisely what F-5b's sandbox backends retire — credentials injected per-execution into an isolated runtime and revoked on reclaim (Spawn's `globalOutbound` pattern). **The forge-credential half of this caveat is now closed** by the fail-closed rule above: the agent can no longer inherit the principal's *GitHub* identity by omission. What remains is OS-level ambient authority, which is a sandboxing concern, not a credential one.
