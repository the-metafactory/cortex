...
- **FAIL CLOSED when no scoped identity is configured (cortex#2436, 2026-07-26).** If `CORTEX_DEV_GH_TOKEN` is unset, `wireDevConsumers` returns **no consumers**: nothing binds, nothing pushes, and the dev loop is inert on that stack until an identity is configured (queued dispatches stay on the stream and run once it is).
