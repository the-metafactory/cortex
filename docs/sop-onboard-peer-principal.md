...
> **JetStream requires a system account.** A cortex stack runs **JetStream by default** (the `jetstream{}` block in your bus config). In operator mode JetStream needs a configured **`system_account`** for its internal `$JS`/`$SYS` subscriptions — without one, `nats-server` fatals at boot with `Can't start JetStream: setting up internal jetstream subscriptions failed: system account not setup`. So for any JetStream-enabled stack the SYS account is **required, not optional**: create it once with `nsc add account SYS`, and `cortex network provision` (Step 4a) then carries `system_account[_jwt]` into `stack.nats_infra` so `make-live` / `join` render it for you. It is only optional if you remove the `jetstream{}` block.
