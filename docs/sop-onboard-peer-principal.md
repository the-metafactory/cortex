...
> **JetStream requires a system account.** A cortex stack runs **JetStream by default** (the `jetstream{}` block in your bus config). In operator mode JetStream needs a configured **`system_account`** for its internal `$JS`/`$SYS` subscriptions — without one, `nats-server` fatals at boot with `Can't start JetStream: setting up internal jetstream subscriptions failed: system account not setup`. So for any JetStream-enabled stack the SYS account is **required, not optional**: create it with `nsc add account SYS` and ensure `system_account` + `system_account_jwt` are present in your stack's `nats_infra` block. `cortex network provision` (Step 4a) exports them when the SYS account already exists in your operator at provision time; if you add SYS afterwards, set those two fields yourself. `make-live` / `join` then render the system account into the bus config — verified end-to-end on a fresh stack (`nats add account SYS` → fields in `nats_infra` → `make-live` rendered `system_account` + the boot fatal cleared). It is only optional if you remove the `jetstream{}` block.
