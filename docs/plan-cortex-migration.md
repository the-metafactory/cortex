...
- [x] **1.10** Add the fail-safe rule check at config-load: `~/.config/cortex/cortex.yaml` `renderers[]` must provide ≥2 distinct platform classes covering `local.{org}.system.>`. Refuse to start otherwise. *(cortex#1893, merged — `assertConfiguredSystemCoverage` in `src/renderers/coverage.ts`, decision ratified as ADR-0024 §OQ9. **Known divergence:** the implementation counts distinct `renderers[].kind` strings, not the `chat-gateway` / `webhook-out` / `paging` / `local-projection` platform classes the rule means; see the ⚠ gap note under ADR-0024 §OQ9 "Platform classes".)*
