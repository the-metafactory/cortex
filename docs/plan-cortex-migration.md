...
- [x] **1.10** Add the fail-safe rule check at config-load: `~/.config/cortex/cortex.yaml` `renderers[]` must provide ≥2 distinct platform classes covering `local.{org}.system.>`. Refuse to start otherwise. *(cortex#1893 merged the guard; **cortex#2503** made it count platform classes rather than `kind` strings, which is what this step's acceptance actually asks for — `discord`+`slack` is now correctly refused as one class, and an unclassifiable kind is never counted toward diversity, and blocks boot only when the classified renderers do not already meet the floor without it. Decision: ADR-0024 §OQ9.)*
