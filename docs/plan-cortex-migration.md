...
- ~~`docs/design-event-taxonomy.md`~~ — **never ported into cortex.** The grove-era event-taxonomy draft ("G-1111", Andreas, 2026-05-09) lives only on `the-metafactory/grove-v2` PR [#81](https://github.com/the-metafactory/grove-v2/pull/81), branch `feat/g-1111-event-taxonomy`, still open. Cortex deliberately did **not** adopt it (2026-08-11): its per-surface grove YAML config model no longer exists, and its §4.6.1 mandatory-`local-projection` floor is overruled by ADR-0005 §4 + ADR-0024 §OQ9. Its still-binding parts were carried over to four different homes — see the erratum table in **ADR-0024** for the authoritative list (platform classes → §OQ9; **reload re-enforcement → cortex#2504, deliberately NOT into the ADR**; self-emission → `src/bus/system-events.ts`; no-direct-NATS-subscription → `docs/architecture.md` §3.4). The event vocabulary itself is owned by `docs/architecture.md` §3 and `src/bus/{system,dispatch}-events.ts`. This entry is kept as the provenance record — do not re-add the pointer.
...
2. **Conflated layers.** Today's `src/bot/` is simultaneously the L7 surface adapter (Discord I/O), the L4 coordination runtime (myelin subscriber, message router), and the workflow runner (CC orchestration, worklog state). The layered concern map says these are three independent concerns. Restructuring them in place inside grove-v2 inherits the brand confusion.
...
| `src/hooks/lib/event-taxonomy.ts` | 74 | tap (in-process taxonomy) | `src/taps/cc-events/event-taxonomy.ts` | **Lives at `src/hooks/lib/`, not `src/bot/lib/` — corrected from earlier draft.** Maps CC hook payloads → internal event-type strings (in-process taxonomy; see `src/taps/cc-events/cc-events.ts`). Moves as part of `src/hooks/` mass-move in MIG-5. |
...
| `src/bot/lib/message-router.ts` | `src/bus/dispatch-handler.ts` | MIG-1 | Move + rename internal class; surface-router lands as new code in MIG-1. **No `inbound-queue.ts` to move** — that file is legacy-grove-only (per §2.2.1); cortex relies on JetStream durability instead. |
...
- [x] **1.9** Implement `cortex/src/bus/surface-router.ts` — the surface-router primitive. Surface adapters register; surface-router subscribes to NATS via `MyelinRuntime` and fans envelopes to registered adapters by subject pattern + payload filter. New code, not a port; tests new. *(cortex#25 + #27, merged 2026-05-09 — MIG-1.12 surface-router primitive + runtime wiring)*
- [ ] **1.10** Add the fail-safe rule check at config-load: `~/.config/cortex/cortex.yaml` `renderers[]` must provide ≥2 distinct platform classes covering `local.{org}.system.>`. Refuse to start otherwise. *(**Partially shipped, deliberately still unchecked.** cortex#1893 merged `assertConfiguredSystemCoverage` in `src/renderers/coverage.ts` and the decision was ratified as ADR-0024 §OQ9 — but the guard counts distinct `renderers[].kind` strings, NOT the `chat-gateway` / `webhook-out` / `paging` / `local-projection` platform classes this step's acceptance names. `discord`+`slack` therefore passes as "two classes" while being one. The step's stated acceptance is unmet until cortex#2503 lands; see the ⚠ gap note under ADR-0024 §OQ9 "Platform classes".)*
...
- A simulated shard disconnect emits `system.adapter.degraded` after threshold; the dashboard renderer (mandatory subscriber per ADR-0024 §OQ9) renders the degraded state.
...
- [x] **4.6** Runner emits lifecycle envelopes per `docs/architecture.md` §7.3 (`dispatch.task.{started,completed,failed,aborted}`). *(cortex#30, merged 2026-05-09 — MIG-4b dispatch.task.* lifecycle envelopes emitted)*
...
- [x] **7.2d** Implement `Renderer` interface + dashboard renderer + pagerduty renderer (the ADR-0024 §OQ9 fail-safe rule's recommended pair). cli-tail renderer optional for v1. *(cortex#50, merged 2026-05-11 — Renderer interface + DashboardRenderer + PagerDutyRenderer; cli-tail deferred per plan caveat)*
...
- [x] **7.6b** Rename `src/taps/cc-events/grove-relay.ts` → `src/taps/cc-events/relay.ts` (or similar cortex-internal name). The file was lifted unchanged at MIG-5a per plan §1.3 "no behaviour change" so principals upgrading still see familiar `grove-relay: …` log lines and `~/bin/grove-relay` symlink. MIG-7.6b updates: filename, Commander program name (`.name("grove-relay")`), 9 user-visible log strings (`"grove-relay: …"`), and the postupgrade.sh symlink target. Coordinated with 7.6a deployment rename and 7.7 arc-manifest provides update so that `~/bin/grove-relay` and `~/bin/relay` (or whatever the new binary is named) co-exist briefly during cutover. (Echo round-1 m2 on cortex#17 flagged the deferral; this checkbox closes the loop.) *(cortex#39, merged 2026-05-11 — `grove-relay.ts` → `relay.ts` + Commander program name + log strings + symlink target to `~/bin/cortex-relay`)*
- [x] **7.7** Update `arc-manifest.yaml`: *(cortex#52, merged 2026-05-11 — arc-manifest provides surface: cortex + cortex-relay + discord + cldyo-live + Discord skill + hooks; lifecycle scripts wired)*
  - `name: Cortex`
  - `version: 0.1.0`
...
- [x] **7.10** Generate cortex's CLAUDE.md via `arc upgrade compass` (or copy + fill). *(cortex#55, merged 2026-05-12 — built `agents-md.yaml` + 7 `docs/agents-md/*.md` section files. Generator lives at `~/Developer/arc/src/lib/rules.ts:generateRules()` and runs as part of `arc upgrade <rules-package>`. Idempotency verified — re-running produces byte-identical CLAUDE.md.)*
- [x] **7.11** Move all `docs/design-*.md` and `docs/iteration-*.md` from grove-v2 → cortex `docs/`. Archive obsolete ones. *(cortex#54, merged 2026-05-11 — 28 design-*.md + 17 iteration-*.md lifted with HTML-comment provenance headers; `v1-to-v2-cutover.md` archived to `docs/archive/` with STALE banner.)*
- [x] **7.12** Move `THIRD-PARTY-NOTICES.md`, `LICENSE`, `README.md`. *(cortex#54, merged 2026-05-11 — LICENSE = MIT byte-identical to myelin/compass ecosystem default; THIRD-PARTY-NOTICES.md re-anchored Grove→Cortex; README.md complete rewrite for v1.0.0 cutover candidate framing.)*
...
- **Lineage docs** — `docs/design-collaboration-surface.md` (PRs #58 + #83), `docs/iteration-collaboration-surface.md` (PR #79).
- **Stack model** — `~/Developer/myelin/specs/namespace.md`, `~/Developer/myelin/schemas/envelope.schema.json`, myelin#7 (Myelin layer model, M1–M7), `~/Developer/myelin/docs/design-agent-task-routing.md` (myelin PR #36 — distribution modes + stratification + lifecycle), myelin#31 (chain-of-stamps).
...
- **Pre-cutover PRs in grove-v2** — `the-metafactory/grove-v2#82` (Discord outage resilience; lands pre-MIG-3), `the-metafactory/grove-v2#81` (event taxonomy — never ported; see §Reading above), `the-metafactory/grove-v2#79` (G-1100 ladder retro), `the-metafactory/grove-v2#84` (this plan + design-cortex.md).
