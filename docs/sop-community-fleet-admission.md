> **Scope correction (ADR-0015).** This SOP covers **Tier-1 (chat) admission only** — the `community-fleet` Discord role (presence + channel access, **no bus**). The bus-side references below to `cortex creds issue`, the `register → issue → join` O-4 handshake, and `runbook-leaf-cred-issuance.md` are **retired hub-minted-identity machinery** ([ADR-0015](./adr/0015-two-tier-onboarding-and-admission-gate.md) dropped hub-minted guest credentials). There is **no guest-bus tier**: a member who wants bus participation runs their **own** sovereign stack — see [`sop-onboard-peer-principal.md`](./sop-onboard-peer-principal.md) (Tier 2) + [`personas/pier.md`](../personas/pier.md). Read the Discord **role mechanics** here as current; treat the hub-minted-identity bus-cred steps as historic.
