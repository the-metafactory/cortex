...
Critically: a lot of F-10's plumbing already exists. The backend `POST /api/assignments/:id/input`, the WS `principal.input` broadcast, the `state.queues` + `sendInput` + `releaseQueue` trio in the dashboard, and the F-7 drill-down's disabled textarea placeholder are all in place from earlier phases. F-10 is primarily a **wire-up**, not a greenfield build. This addendum keeps the scope honest to that reality.
...
F-7 ships `#drill .drill-input textarea` as a disabled placeholder (Decision 1 of the F-7 addendum). F-10 enables it. The v1 PR#1 placeholder `.list/.detail` panel in `<main>` keeps its own input, which is already functional against the same endpoint — removing it is a **Phase C cleanup** that can land in a small follow-up chore PR after F-10 has had some use. Scope-avoidance is not the intent; leaving the placeholder gives the principal a working escape hatch if the drill-down overlay has a regression post-merge.
...
2. Inputs are low-frequency (principal keystrokes, not tool call streams). A round-trip latency of 20 ms over HTTP is imperceptible.
...
`state.queues` already exists, typed `Map<assignmentId, { busy: boolean, queue: string[] }>`. `sendInput` / `releaseQueue` are already implemented in line with §6.3. F-10 renders the queue depth **in the drill-down input area** (where the principal sees it) rather than only in the v1 placeholder `.detail` column (where they have to look for it).
...
- `Retry` re-submits the same text (preserves what the principal typed).
...
The global `showError` is still called so it's visible if the principal closes the drill-down; inline banner is additive. Matches the F-8 review learning (grid-refresh errors stopped using `showError` because they over-shadowed F-6/F-8 errors — PR #15 sweep). Different case here (input failure is itself an action the principal just took, so it belongs inline), but the principle of "put the error where the principal is looking" is the same.
...
**Empty-assignment tasks** (the F-8 Decision 5 case where the principal opened the drill-down on a task with zero assignments): the drill-down doesn't open in that case — F-8 already surfaces "Manual dispatch ships in Phase E (F-12)" as an error-pill message, and the drill-down never gets an assignment ID to work with. Not an F-10 concern.

## Decision 10 — Audit-log aggregation stays as-is; `principal.input` is already the first-class event

§6.4 specifies "full audit log of principal ↔ agent interactions, across all agents", implemented as `principal.input` events in the `events` table. That is **already true** post-F-3. The existing event stream in `events` is the audit log. The F-7 attention view already renders `principal.input` as an H-coloured primary row in the event log.

**F-10 does not need to add aggregation surface.** An principal-across-agents view ("show me everything I typed today") is either:

- Implicit via the F-8 table (click any task, see the drill-down events).
- Or a dedicated per-principal event feed — post-Phase-B at earliest, no principal has asked for it, and adding it now is scope creep.
...
- Per-principal cross-agent audit view (Decision 10 — implicit via F-7/F-8 already; dedicated view is post-Phase-B).
...
- [ ] Submit routes through `sendInput`; principal sees optimistic "Sending…" during the call.
- [ ] On success, textarea clears; the posted text appears in the event log as an `principal.input` H-coloured row within one WS tick.
