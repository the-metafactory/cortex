...
**The second thing that bites — a stale leaf secret (clear + reissue).** Even once the member's bus is operator-mode, a **stale/mismatched sealed PSK still on their registry row** makes every `cortex network join --apply` re-fetch + re-render it as an inline-PSK leaf the hub rejects — an **Authorization-Violation storm on each attempt** (recovering forces a restart back onto the safe config). Root cause is usually a member who was on a `$G`/creds-only bus when the PSK was first sealed (there was no sovereign FED account for it to bind to). Fix, **Model-B only**: (1) the member converts to operator-mode with their **own** FED account (above) and sends you their **FED pubkey**; (2) you **re-run `cortex network secret add-member <net> <member-FED-pubkey> --admin-seed <hub-admin-seed> --hub-config <the real hub> --apply`** — it re-mints a fresh PSK, **re-seals over the stale row**, and re-writes the hub authorization; (3) the member re-runs `join`. **Never** issue the member a creds/hub-side account to sidestep this — that is the **rejected Model A** ([ADR-0013](./adr/0013-sovereign-federation-model.md)). When the hub is another principal's server, the hub-authorization write is theirs (two-party, [ADR-0018](./adr/0018-admission-gate-and-leaf-secret-distribution.md)).
