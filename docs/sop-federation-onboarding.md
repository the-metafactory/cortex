...
1. **Leaf-account binding (code, mechanical) — [#1261](https://github.com/the-metafactory/cortex/issues/1261).** Re-point each network's leaf remote off the shared `ANDREAS_AGENTS` account onto its **per-stack federation account** (`ANDREAS_<STACK>_FED`, already minted by provision) via the federated `cortex network join` flow. Completes Model-B leaf sovereignty ([ADR-0013](./adr/0013-sovereign-federation-model.md) §Decision-3): networks isolate by subject scope + per-stack federation account, never by a shared account.
