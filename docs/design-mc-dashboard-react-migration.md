...
**Addendum to:** `docs/design-mission-control.md` §8 (Primary attention focus), §6 (Principal input channel), and all prior F-6…F-10 / image-input addenda.
...
**Date:** 2026-04-25 (revised after principal-feedback course correction).
...
| F-10 | Drill-down principal text input + queue + inline error banner | PR #17 |
...
1. **The mission-control spec** (`docs/design-mission-control.md`) — principal-first, keyboard-driven cockpit with a focus row, working-agent grid, task table, and drill-down. §6 is explicit: the drill-down is a full principal-input channel, not a read-only view.
...
These three are not three separate designs — they converge on the same thing. The spec says "drill-down"; the handoff's V4 says "re-orientation"; the principal says "CC session parity." Read together, the target is one surface — a rich session view — wrapping the existing event log + principal input with plan/progress, artifact access, and a conversation-first interaction model.

**What "CC session parity" concretely means** (surfaced by mining this very conversation — the way the principal course-corrects mid-stream, asks for plan visibility, drops references like "can we do React", "where are we at", "did you check the design file", is exactly the pattern the drill-down must support):
...
- **Artefact links.** Branch, PR URL, diff summary, issue references — pulled from the agent's activity into a header strip. Lets the principal jump-to-context without leaving the drill-down.
...
- **Actionable permission prompts.** Approve/Deny inline on `permission.request` rows, with room for the principal to attach reasoning ("yes, but only if you also update the test") — text passes through as the "deny with instructions" channel until CC's permission-protocol stream-json verification lands (F-7 Decision 6 gate still holds).
- **Single-agent focus mode.** Full-screen the drill-down; keyboard shortcut to cycle between blocked agents.

**Principal's primary surface is the dashboard, not the CLI.** The CLI remains valid for exploratory work and `cldyo-live` observation, but the principal who drops into the mission-control page at `/` is doing their real work there. The drill-down UX must not be a strictly-worse proxy for a CC terminal.
...
- Principal has explicitly reframed the drill-down: V4-flavoured session-context is the primary drill-down, not a deferred variation. This addendum's Decision 7 and the MIG-3 scope are revised accordingly below.
...
2. **The design artifact + principal feedback are aligned.** The Claude Design handoff ships a five-variation prototype with a shared visual vocabulary (keycaps, pills, event rows, focus cards, command palette). The principal's 2026-04-25 reframing made V4 Session Context the primary drill-down rather than a sibling variation. Re-implementing any of this in vanilla JS compounds the monolith problem.
...
- **Dev:** Bun's `--watch` mode rebuilds on save. Manual browser reload. Acceptable given the dashboard is used loopback by one principal at a time — HMR is not load-bearing.
...
**Accepted cost.** Bun bundler's React Fast Refresh support is less mature than Vite's. Full reload on save is a minor DX regression for 1 principal.
...
**Not chosen.** Redux, Zustand, Jotai, Valtio — all add a library for state the dashboard can handle with React primitives. At the dashboard's size (5 features, single principal) a store library is ceremony. Revisit if the app grows past ~10 features or we need time-travel debugging.
...
- **Light theme** is the design artifact's light palette (off-white backgrounds, dark text, same hue families). Default is whichever matches the principal's OS preference; explicit toggle persists to `localStorage.theme`.
...
- Canned action buttons below the reply surface: "Ask for more", "Show test output", "Review PR", "Redirect focus". Each is a pre-filled text that the principal can edit before sending (buttons are shortcuts, not commitments).
...
- Canned-button customisation / principal-defined shortcuts.
...
- **Accent-hue slider** — debug affordance, not principal-facing.
...
- **Component smoke tests** — deferred to a follow-up. Bun test doesn't ship a DOM adapter out of the box; the cost-benefit of wiring `happy-dom` + `@testing-library/react` is low for a single-principal app where manual testing is already the QA method. Revisit if component regressions become a pattern.
...
- Principal-visible risk is zero: the monolith at `/` keeps working while the port progresses at `/v2`. If a port regresses, the principal still has `/`.
...
**Accepted cost.** The `/v2` route runs alongside `/` for the duration of the migration (6–8 PRs). The principal can A/B compare. At cutover, the monolith is deleted and `/v2` becomes `/`.
...
- **Principal impact:** zero. `/v2` shows the shell-only app; `/` is unchanged.
...
- **Principal impact:** `/v2` now shows the focus row. Backward-compat at `/` unchanged.
...
**Principal impact:** `/v2` has a full V4-flavoured drill-down that feels like the CC terminal for that agent. Input + image + queue work. `/` unchanged.
...
- **Principal impact:** `/v2` has focus row + table + drill-down (read-only). Usable for monitoring.
...
- **Principal impact:** `/v2` now has full monitor-mode parity with `/`.
...
- **Principal impact:** `/` is the new dashboard. Rollback is a single revert.
...
- **PR 11+ — V2 Triage + V3 Canvas layouts** if principal demand appears.
...
Added 2026-04-25 to resolve what "feels like a CC session" means *concretely* — turning the principal's observation into buildable criteria for MIG-3. Mined from the actual CC-session interaction pattern this design conversation has been running on.
...
| `stream-json.user` → `text` block | **Suppressed.** `principal.input` is the authoritative H-source (F-7 Decision 4). |  — | — |
| `principal.input` | Primary-weight row with principal's text rendered as markdown. If images attached, rendered inline below the text (see image-input Decision 7). | — | H (rose). |
...
When an item transitions `pending → in_progress → completed`, the pane briefly highlights that row for ~800ms so the principal catches which step just landed.
...
Fixed v1 set (all pre-fill the textarea; principal edits before sending):
...
- **Redirect focus** → "Park this and switch to [ ]." (principal fills the blank)
...
- `]` / `[` cycle between *blocked* assignments in the current focus-row order. Principal never leaves the drill-down while unblocking work.

### Interaction model — why this matters

The live CC CLI session gives the principal: transcript, tool-call visibility, plan state, permission prompts with context, rich reply, paste, voice (via desktop OS), Ctrl+C / Esc escape hatches, history scrollback. The dashboard drill-down today gives: flat event log with D/A/H colour, disabled Approve/Deny, textarea, paste images.

Decision 11 closes that gap per row above. MIG-3 ships this; the migration is not "done" (or even reviewable as a drill-down port) without it. V4 from the design artifact was the principal's reach toward this target; the principal's explicit 2026-04-25 feedback confirmed it.
