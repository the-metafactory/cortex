 ...
 * cortex#1795 (S10, ADR-0024 D2/D5 extraction lane) — this file (and
 * `./index.ts`) import ONLY from `../../surface-sdk` across the
 * `src/adapters/slack/` boundary (plus intra-directory `./index` /
 ...
 * renderer became a host-injected function. This makes `src/adapters/slack/`
 * compilable in isolation against `surface-sdk` alone — the precondition
 * for relocating it out of this repo (mirrors cortex#1794 S9b/S9's identical
 * pass for `web`).
 *
