...
describe("transparent upgrade E2E — the real metafactory-cortex-adapter-web bundle loads flag-OFF as first-party (cortex#1794 S9)", () => {
  ...
  test("the loaded plugin registers into the SAME registry a discord stub already lives in, alongside it", async () => {
    // cortex#1795/#1796/#1797 (S10/S11/S12 MOVE) — "slack", "mattermost", AND
    // "discord" are all out-of-tree now; the in-tree default registers ZERO
    // adapters. Register a discord stand-in ourselves so this test still
    // proves what it always meant to: a second platform (`web`) registers
    // into the SAME registry instance alongside an already-present one,
