#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
# Cortex Status Line  —  see cortex#96
# ═══════════════════════════════════════════════════════════════════════════════
#
# Lifted from grove's PAI-flavored statusline as cortex takes over grove's
# operator-facing surfaces (cortex#96). Every PAI-path lookup is optional:
# if $PAI_DIR is unset or the referenced file/dir doesn't exist, the section
# that depends on it is silently skipped. The script produces a valid (if
# minimal) status line with zero PAI deps reachable.
#
...
eval "$(echo "$input" | jq -r '
  ...
  "cc_rate_5h=" + (.rate_limits.five_hour.used_percentage // "" | tostring) + "\n" +
  "cc_rate_5h_reset=" + (.rate_limits.five_hour.resets_at // "" | tostring) + "\n" +
  "cc_rate_7d=" + (.rate_limits.seven_day.used_percentage // "" | tostring) + "\n" +
  "cc_rate_7d_reset=" + (.rate_limits.seven_day.resets_at // "" | tostring)
' 2>/dev/null)"
...
# Override usage with Claude Code's live rate_limits data when present.
# The cached API data can be stale (cached file persists after window resets).
# Claude Code provides real-time rate_limits in the input JSON — always prefer it.
_cc_has_live=false
if [ -n "$cc_rate_5h" ] && [ "$cc_rate_5h" != "null" ] && [ "$cc_rate_5h" != "" ]; then
    usage_5h="$cc_rate_5h"
    [ -n "$cc_rate_5h_reset" ] && usage_5h_reset="$cc_rate_5h_reset"
    _cc_has_live=true
fi
if [ -n "$cc_rate_7d" ] && [ "$cc_rate_7d" != "null" ] && [ "$cc_rate_7d" != "" ]; then
    usage_7d="$cc_rate_7d"
    [ -n "$cc_rate_7d_reset" ] && usage_7d_reset="$cc_rate_7d_reset"
    _cc_has_live=true
fi

# Write CC live data back to cache so EventLogger hook + grove-bot pick it up.
# Converts CC format (used_percentage, epoch resets_at) → API format (utilization, ISO resets_at).
if [ "$_cc_has_live" = "true" ]; then
    python3 -c "
import json, os
...
cc5h, cc5r, cc7d, cc7r = '$cc_rate_5h', '$cc_rate_5h_reset', '$cc_rate_7d', '$cc_rate_7d_reset'
if cc5h:
    d.setdefault('five_hour', {})
    d['five_hour']['utilization'] = int(float(cc5h))
    if cc5r: d['five_hour']['resets_at'] = epoch_to_iso(cc5r)
if cc7d:
    d.setdefault('seven_day', {})
    d['seven_day']['utilization'] = int(float(cc7d))
    if cc7r: d['seven_day']['resets_at'] = epoch_to_iso(cc7r)
...
detect_terminal_width() {
    ...
    # Wrap full subshell in 2>/dev/null so the </dev/tty redirect error
    # itself (emitted by bash before stty runs) doesn't leak to stderr
    # when invoked without a controlling tty (cortex#96).
...
# ═══════════════════════════════════════════════════════════════════════════════
# EXTENSIONS: cortex-status.sh + statusline.d/*.sh
# ═══════════════════════════════════════════════════════════════════════════════
# Cortex installs its helper at $PAI_DIR/cortex-status.sh (sibling of this
# script). Source explicitly when present; silently skip if missing so the
# statusline still renders on hosts without cortex installed (cortex#96).

if [ -n "$PAI_DIR" ] && [ -r "$PAI_DIR/cortex-status.sh" ]; then
    source "$PAI_DIR/cortex-status.sh"
fi

# Forward-compat: also auto-source any *.sh dropped into statusline.d/
if [ -n "$PAI_DIR" ] && [ -d "$PAI_DIR/statusline.d" ]; then
    for _ext_sh in "$PAI_DIR/statusline.d"/*.sh; do
        [ -r "$_ext_sh" ] && source "$_ext_sh"
    done
fi
