 ...
 *   - `home_principal`   — `signed_by[0].principal` principal segment with
 *     the `did:mf:` prefix stripped. Drives D.5.2 per-principal slicing.
 *     The cloud worker also keeps `sessions.operator_id` (the API-key
 *     owner) which is the *receiving* principal; `home_principal` is who
